...
过期后**允许登录与切换租户**，会话保留；`AuthUser.tenant_expired=true`。提示文案：「套餐已过期，请联系管理员续费」。

| 能力 | 过期后行为 |
|---|---|
| 查询 / 列表 | 只读；列表最多前 **500** 条；`page>1` 返回 403 |
| 新增 / 编辑 / 删除 / 导出 | 禁止（403）；前端立即弹窗（不受 4 小时缓存限制） |
| 定时任务 | 该租户的租户级任务不执行；手动触发报错 |
| 进入提醒 | 登录成功后弹窗；关闭后 **4 小时**内不再自动弹出 |
| 平台续费 | `PUT/POST /api/v1/tenants`、`/api/v1/orgs` 仍允许（便于续期） |

- 定时任务 `tenant_expiry_check`：扫描过期租户并将会话标记为软锁定（**不踢下线**）。
- 前端约每 5 秒探测 `/auth/me` 同步 `tenant_expired`；详见 [AGENTS.md](../AGENTS.md)「租户套餐过期」。
